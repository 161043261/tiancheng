...
- public 下的文件, 不会被 vite 打包
- src/assets 下的文件, 会被 vite 打包
...
- 计算属性会缓存计算结果, 只有当依赖项更新时, 才会重新计算 (基于脏值检测)
...
// 订阅
emitter.on("event-name", (...args: unknown[]) => console.log(args));
// 取消订阅
...
Teleport 将部分 template 传送到指定 CSS 选择器的元素下, 成为该元素的「直接子元素」
