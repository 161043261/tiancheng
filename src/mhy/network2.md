...
长轮询是指浏览器发送请求时, 服务器保持连接, 等到有新消息时才返回, 减少了请求次数, 提高了实时性, 缺点:

1. 多线程服务器大部分线程在等待新消息, 即大部分时间挂起, 浪费 CPU 资源
...
先试图命中强缓存, 再试图命中协商缓存, 强缓存和协商缓存的相同点是如果命中, 是从客户端缓存中加载资源, 不是从服务器加载资源; 不同点是强缓存不会发送请求到服务器, 协商缓存会发送请求到服务器; 当协商缓存也未命中时, 从服务器加载资源
...
- chrome 为每一个页面创建一个渲染进程, 所以一个页面崩溃不会影响其他页面
...
1. 解析 HTML, 深度优先遍历以构建 DOM 树
   - 遇到 `<style>` 或 `<link>` 标签会同时加载, 解析 CSS, 构建 CSSOM 树
   - 遇到未使用 async 或 defer 标记的 `<script>` 标签, 会阻塞 DOM 树的构建, 并等待 CSSOM 树构建完成后, 再转而加载, 解析并执行 JS 脚本
...
3. 布局与绘制
4. 重绘 (repaint 有关颜色的..., 性能开销小) 和回流 (reflow 重排, 有关宽高的..., 性能开销大)
...
1. 加载, 解析 CSS, 构建 CSSOM 树不会阻塞 DOM 树的构建
2. 等待 CSSOM 树构建完成后, 才能将 DOM 树和 CSSOM 树合并为渲染树 (Render Tree)
3. 等待 CSSOM 树构建完成后, 才能加载, 解析并执行后续的 JS 脚本

### JS 的阻塞

浏览器解析 HTML 时, 遇到未使用 async 或 defer 或 `type="module"` 标记的 `<script>` 标签, 会阻塞 DOM 树的构建, 并等待 CSSOM 树构建完成后, 再转而加载, 解析并执行 JS 脚本
